...
function getCartTotalAmount() {
  let result = 0;
  SHOPPING_CART.forEach((s) => {
    PRODUCTS.forEach((n) => {
      if (s.id === n.id) {
        result += (n.price * s.quantity);
      }
    });
  });
  return result;
...
function removeProductFromCart(productId) {
  ...
  if (itemIndex !== -1) {  // If the product exists in the cart
    const item = SHOPPING_CART[itemIndex];

    if (item.quantity > 1) {
      // If quantity is greater than 1, just decrement the quantity
      item.quantity--;
    } else {
  ...
  // If the product doesn't exist in the cart, do nothing
}
